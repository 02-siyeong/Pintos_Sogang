...
page_fault (struct intr_frame *f) 
{
  ...
  user = (f->error_code & PF_U) != 0;
	...
	if (fault_addr < PHYS_BASE && not_present)
		{
			struct page * p = page_lookup (&t->pt, (void *)((int)fault_addr & 0xfffff000));
		
			if (p != NULL && p->is_swapped)
				{
					if (swap_lookup (p->sector_num, NULL))
						{
							uint8_t *kpage;
							...
							frame_insert (t, p->upage, kpage);
	
							swap_lookup (p->sector_num, kpage);
	...
	if (fault_addr >= f->esp - 32  && fault_addr < PHYS_BASE)
		{
			void * upage = (void*)((int)fault_addr & 0xfffff000);
			uint8_t *kpage;
			...
			if (pagedir_get_page (t->pagedir, upage) == NULL
				...
				{
					page_insert(&t->pt, upage, kpage, true);
					return;
				}
		}
	sys_exit (-1);
